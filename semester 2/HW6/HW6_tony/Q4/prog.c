...
//struct bingoPlayer -->add typedef to use the struct with name
typedef struct bingoPlayer
{
	//char name[STR_LEN] = "Daniel"; --> cannot init variabel in struct init
	//char board[BOARD_SIZE][BOARD_SIZE] = {0}; --> cannot init variabel in struct init
	char name[STR_LEN];
	char board[BOARD_SIZE][BOARD_SIZE];
}bingoPlayer;//add name to struct (because of typedef)

typedef struct bingoGame
{
	//cannot init variabel in struct init
...
void initBingoPlayer(bingoPlayer *pPlayer)
{
	...
	//myFgets(pPlayer.name, STR_LEN); --> pPlayer name is pointer (for pointer we use '->')
	...
	for (i = 0; i < BOARD_SIZE; i++)
	{
		...
		{
			//pPlayer.board[i][j] = 0; --> pPlayer board is pointer (for pointer we use '->')
			pPlayer->board[i][j] = 0;
		}
	}
...
{
	int i = 0;
	//for (i = 0; i < game->numPlayers; i++) --> numPlayers not pointer (for numbers we use '.' insted of "->")
	for (i = 0; i < game.numPlayers; i++)
	{
		//printf("%s's board:\n", game->players[i]->name); --> not pointer 
		//printBoard(game->players[i]); -- not pointer
		printf("%s's board:\n", game.players[i].name);
